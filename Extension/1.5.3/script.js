$(function() {
	...
	function htmlShowButton(){
		return "Show <img height='10px' src='" + chrome.extension.getURL('images/down.png') + "' />";
	}
	
	function htmlHideButton(){
		return "Hide <img height='10px' src='" + chrome.extension.getURL('images/up.png') + "' />"; 
	} 
	
	function routine(type, selector){
	
		$( selector ).each(function() { 
			var wrapper = $( this ).closest(".fbUserContent, .userContentWrapper ");
	...
	window.setInterval(function(){
		...
		if(pages){
			...
			routine("fan page", ".PageLikeButton");  
		}
		if(videos){
			routine("video" , "video");
		}
		
		$(".ego_unit_container, #pagelet_canvas_nav_content").css('visibility', 'hidden');
		$(".home_right_column").css("min-height", 0);
		$(".ego_unit_container").css("height", 0);
